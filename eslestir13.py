...
def tablo_yukle():
    ...
    for k, v in ham.items():
        ...
        for m in ("migros", "ozdilek", "macro", "carrefour"):
            if isinstance(v.get(m), dict):
                kayit[m] = v[m]
        # "bu markette yok" isaretlerini KORU. Onceden atiliyordu ve her
        # oturumda ayni sorular yeniden soruluyordu.
        for anahtar, deger in v.items():
            if anahtar.endswith("_yok") and deger:
                kayit[anahtar] = True
                korunan_yok[0] += 1
        if kayit:
            yeni[k] = kayit
    ...
    if korunan_yok[0]:
        print(f"  {korunan_yok[0]} 'bu markette yok' isareti korundu")
    return yeni
...
def soru(baslik, adaylar, hedef_m):
    ...
    while True:
        c = input(f"{baslik} (1-{len(adaylar)} / 0=yok / s=sil / "
                  f"z=geri): ").strip().lower()
        if c in ("b",):
            raise Bitti("bitir")
        if c in ("a", "g"):
            raise Bitti(c)
...
def calis():
    ...
    print("         z=bir onceki urune don   a=urunu atla")
    print("         g=grubu atla   b=bitir")
    print("Her karar kaydedilir; ayni soru bir daha sorulmaz.\n")

    atlanan_gruplar = set()
    gecmis = []          # geri gitmek icin: (indeks, urun_id, onceki kayit)
    i = 0
    while i < len(hedefler):
        urun_id, v = hedefler[i]
        sayac = i + 1
        # bu urunun tablodaki hali; 'z' ile geri donulurse buraya donulur
        onceki_hal = copy.deepcopy(tablo.get(urun_id))
        ad = v.get("urun_adi", "")
        ...
        try:
            for kod, baslik, ara in MARKETLER:
                ...
                if kod in kayit:
                    ...
                    # onay=True: kullanici gramaj farkina ragmen bu eslesmeyi
                    # elle dogruladi; bir daha sorulmaz, temizle.py de dokunmaz
                    if mevcut.get("onay") or carpanli or m_miktar == hedef_m:
                        print(f"\n{baslik}: zaten esli - "
                              f"{mevcut.get('ad','')[:40]}")
                        continue
                    ...
                    print("   0 = MEVCUT KALSIN (kalici onaylanir)")
                    print("   numara = degistir")
                    ...
                    if secilen == "sil":
                        ...
                        # Kullanici bilerek korudu -> kalici onay
                        mevcut["onay"] = True
                        kayit[kod] = mevcut
                        print("   -> ONAYLANDI: bir daha sorulmayacak, "
                              "temizleme araci da dokunmayacak")
                    time.sleep(0.2)
                    continue
                secilen = soru(baslik, ara(sorgu), hedef_m)
            ...
            if str(e) == "bitir":
                break
            if str(e) == "z":
                # bu urunde yapilanlari geri al
                if onceki_hal is None:
                    tablo.pop(urun_id, None)
                else:
                    tablo[urun_id] = onceki_hal
                if gecmis:
                    onceki_i, onceki_id, onceki_kayit = gecmis.pop()
                    if onceki_kayit is None:
                        tablo.pop(onceki_id, None)
                    else:
                        tablo[onceki_id] = onceki_kayit
                    i = onceki_i
                    print("   -> onceki urune donuldu")
                else:
                    print("   -> zaten ilk urundesin")
                continue
            if str(e) == "a":
                tablo[urun_id] = {"atla": True}
                print("   -> bu urun bir daha sorulmayacak")
                gecmis.append((i, urun_id, onceki_hal))
                i += 1
                continue
            if str(e) == "g":
                if kel:
        ...
        gecmis.append((i, urun_id, onceki_hal))
        gecmis[:] = gecmis[-30:]        # yigin sisirmesin
        i += 1
