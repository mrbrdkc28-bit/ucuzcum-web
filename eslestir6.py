...
FB_ADI = {"migros": "migros", "ozdilek": "ozdilek", "macro": "macrocenter"}
...
def soru(baslik, adaylar, hedef_m):
    ...
    while True:
        c = input(f"{baslik} (1-{len(adaylar)} / 0=yok / Enter=sonra): "
                  ).strip().lower()
        if c in ("b",):
            raise Bitti("bitir")
        if c in ("a", "g"):
            raise Bitti(c)
        if c == "0":
            return "yok"          # bu markette yok, bir daha sorma
        if c == "":
            return None           # simdilik gec, sonra tekrar sor
...
def calis():
    ...
    for uid, v in urunler.items():
        ...
        if not isinstance(kyt, dict):
            continue
        for anahtar in ("migros", "ozdilek", "macro"):
    ...
    print("Tuslar:  numara=sec   0=bu markette yok (bir daha sorma)")
    print("         Enter=simdilik gec   a=urunu atla   g=grubu atla   b=bitir\n")
